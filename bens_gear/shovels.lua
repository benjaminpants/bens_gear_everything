...
bens_gear.create_example_item("shovel_example","Example Shovel", "default_tool_bronzeshovel.png")

minetest.register_craft({
		type = "shapeless",
		output = "bens_gear:shovel_head_example 1",
		recipe = {
			"bens_gear:blueprint_shovel", "bens_gear:invalid_material", "bens_gear:invalid_material", "bens_gear:invalid_material"
		}
})
...
minetest.register_craft({
		...
		output = "bens_gear:shovel_example 1",
		recipe = {
		{"","bens_gear:shovel_head_example",""},
...
bens_gear.add_ore_iterate(function(ore_data)
	if (not bens_gear.can_tool_be_made("shovel",ore_data)) then return end
	...
	minetest.register_craft({
		...
		output = "bens_gear:shovel_head_" .. ore_data.internal_name .. " 1",
		recipe = {
			"bens_gear:template_shovel", ore_data.item_name, ore_data.item_name, ore_data.item_name
		}
	})
end)
...
local charm_texture = ""
if (charm_data ~= nil) then
	charm_texture = "^" .. bens_gear.get_viable_tool_texture_no_default("shovel",charm_data.valid_tools)
...
local pick_name = "bens_gear:shovel_" .. ore_data.internal_name .. "_" .. rod_data.internal_name .. "_" .. last_append
local rod_texture = bens_gear.get_viable_tool_texture("shovel","bens_gear_rod_shovel_",rod_data.rod_textures,rod_data.color)
local can_fire = (ore_data.flammable and rod_data.flammable)
local temp_groups = {}

if (can_fire) then
	temp_groups = {shovel = 1, flammable = 2}
	minetest.register_craft({
		type = "fuel",
		recipe = pick_name,
		burntime = 5,
...
local pick_data = {
	...
	tool_capabilities = {
		...
		groupcaps={
			crumbly = bens_gear.multiply_times(ore_data.groupcaps.crumbly,rod_data.speed_multiplier),
		},
...
pick_data.tool_capabilities.groupcaps.crumbly.uses = math.ceil(ore_data.uses * rod_data.uses_multiplier)
...
if (rod_data.pre_finalization_function ~= nil) then
	rod_data.pre_finalization_function("shovel",pick_data,pick_name)
end

if (ore_data.pre_finalization_function ~= nil) then
	ore_data.pre_finalization_function("shovel",pick_data,pick_name)
end
...
minetest.register_craft({
		...
		output = pick_name .. " 1",
		recipe = {
		{"","bens_gear:shovel_head_" .. ore_data.internal_name,""},
